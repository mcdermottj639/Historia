...
guides and a figures gallery (**Learn**), story-first **Games** (flagship:
**Crossroads**, where you become a figure and face their real decisions; plus
a once-a-day **Daily Reckoning** and quick deduction/chronology games), and a
daily **Time Machine**, tied together by a Wisdom-XP/rank/day-streak
progression.
...
- `data/stories.js` — `H_STORIES`: Story Mode narratives
  ...
  events[], figures[]}`.
- `data/crossroads.js` — `H_CROSSROADS` (3 scenarios: Caesar, Cleopatra,
  Washington): the Crossroads decision scenarios `{id, fig, emoji, era, title,
  kicker, minutes, intro, beats[{k, t, scene, choices[{t, real?}], outcome,
  alt}], fate}`. `outcome` is fact-checked history; `alt` is the counterfactual
  and the UI labels it "historians can only guess". One choice per beat has
  `real: 1`. Legend is flagged in-text (the bed-sack, the asp, "Et tu").
...
- `crossroads.js` — the **👑 Crossroads** player (rides the story-player
  overlay CSS): intro → per-beat scene + 2 shuffled choices → reveal
  ("✓ History's path" vs dashed "🔮 The road not taken — historians can only
  guess") → end screen scoring **Historian's Instinct** (real choices
  matched). First completion pays 45 + 10/match XP, replays a token amount;
  per-scenario best in `historia:crossroads`. `window.Crossroads.open(id)`.
- `games.js` — Games hub (Daily card → Crossroads shelf → quick-game grid) +
  **🗓️ Daily Reckoning** (seeded daily mystery event, same for everyone:
  4-clue ladder era→region→~year→blurb, 6 candidates incl. 2 same-era + 1
  same-region decoys; wrong guess −20 and unlocks next clue, extra clue −10,
  pot 100→15; state persists in `historia:daily` so refresh can't re-roll;
  pays score/2 XP once/day) + **🎭 Who Am I?** (progressive name-redacted
  figure clues, 4 suspects, pot shrinks per clue/wrong guess, portrait on
  reveal, best `who`) + **🔮 What Happened Next?** (face-up anchor event with
  blurb; 3 same-region candidates — one really next, two already happened,
  marked ⏮ on reveal; best `next`) + **⚖️ Before or After** (3 lives, streak
  scoring, year-gap ramp `gapFloor`, best `ba`) + **⏳ Timeline Sort** (tap 5
  events earliest-first; min-gap ramp `ROUND_GAP`, +20/pick, +50 perfect,
  first slip per round is a free warning, best `sort`). Chronology games keep
  the **era pill and year hidden until reveal**, but face-down cards **do show
  the blurb** — players guess from the story, not a bare title. Guess the Era
  was cut (another date-guesser); Trivia Lab remains the one "soon" tile.
...
Current version as of this writing: **v12**.

- **v12** — 🎮 **The Games Hall rebuild** (owner: "the games are terrible…
  do it all"). New flagship **👑 Crossroads** (`crossroads.js` +
  `data/crossroads.js`): become Caesar, Cleopatra or Washington and face
  their real decisions beat by beat — choose, then see fact-checked
  "History's path" vs a clearly-labeled speculative "road not taken"; scored
  as Historian's Instinct (matches). New **🗓️ Daily Reckoning**: one seeded
  mystery event a day (same for everyone), 4-clue deduction ladder, elimination
  guessing, state persisted so it can't be re-rolled — wired to the Home
  "Time Machine" button too. New **🎭 Who Am I?** (name-redacted progressive
  figure clues, 4 suspects) and **🔮 What Happened Next?** (anchor moment +
  3 same-region candidates, two of which already happened). Existing games
  softened: face-down cards now show blurbs (guess from the story), Timeline
  Sort gives one free slip per round. Guess the Era cut from the roadmap
  (it was a third date-guesser). Hub reordered: Daily → Crossroads shelf →
  quick games.
...
- `historia:best` — per-game best scores (`ba`, `sort`, `who`, `next`).
- `historia:crossroads` — `{scenarioId: {done, best (instinct matches)}}`.
- `historia:daily` — `{date, clues, wrong[], done, score}` — today's Daily
  Reckoning state (persisted mid-game so refreshing can't re-roll it).
...
Next up, in rough order: **more Crossroads scenarios** (candidates: Lincoln
1861–65, Napoleon 1799–1815, Elizabeth I, Mansa Musa, Genghis Khan) ·
**Trivia Lab port** with history categories + OpenTDB blend · more stories
(goal: 10+) · **SVG map view** (authoring-time geocoding) · per-era mastery ·
`historia:otd` cache pruning if it ever grows beyond one day's entry.
(Guess the Era was deliberately cut — a third date-guessing game.)
